...
-- ENFORCEMENT, NOT AN ACCESS PATH. This comment used to claim the opposite:
-- that the SCD2 loader joins WHERE is_current on every run and "that lookup
-- must be an index scan". The index usage check in 06_performance.sql section
-- 9 disproved it. After a full load and the whole analytics suite this index
-- has been scanned once. The loader does join on is_current, but against
-- roughly 800 rows the planner correctly picks a sequential scan and a hash,
-- and no amount of justification written above the line changes what the
-- planner does.
--
-- It stays because a UNIQUE index is a constraint, and constraints are judged
-- on whether the invariant is wanted rather than on scan counts. The invariant
-- is "at most one current version per broker", stated in a single line a
-- reader can check. It is belt and braces over the EXCLUDE constraint above,
-- which already implies it (two current rows both run to infinity, so their
-- ranges always overlap). 40 kB to state an invariant twice is a trade worth
-- making on the dimension that the whole warehouse resolves through.
...
-- DELETED, AND THE DELETION IS THE POINT. There was an index here:
--   CREATE INDEX ix_dim_broker_agency ON mart.dim_broker (agency_sk) WHERE is_current;
-- justified as "agency level rollups that start from the outrigger and come
-- back down to brokers". The index usage check in 06_performance.sql section 9
-- measured zero scans on it after a full load and every analytics query. The
-- rollup that would have used it, Q13, walks dim_agency recursively and joins
-- the pipeline fact directly, so it never comes back down through dim_broker
-- at all. The justification described a query nobody had written.
--
-- The rule this file states is that a zero scan index is either dead weight to
-- delete or a missing query to write. This one was dead weight. Deleting it is
-- cheaper than inventing a query to keep it alive, and leaving it with the
-- comment intact would have been the worst of the three options.
...
-- count, and the worst case is a generic parameterised plan that has to
-- carry every partition because it cannot know the parameter yet. That cost
-- is measured in PERFORMANCE.md, section "The cost side of partitioning",
-- which is the one place in this repository the figures live. Quoting the
-- cost and not only the win is what makes this a decision rather than a
-- fashion.
--
...
-- A BRIN on event_ts was built on this exact 1,165,043 row fact and it WAS
-- used by the planner, and it DID help: it is the difference between reading
-- every partition and reading the ranges that can contain March. It was
-- rejected anyway, on a straight head to head against the alternative.
--
-- The alternative is free. The filter that wants a BRIN is on event_ts, and
-- event_ts is not the partition key. Rewriting the same predicate onto
-- event_date, which IS the partition key, gets partition pruning instead:
-- fewer buffers, one partition instead of 85, and no index to build, store or
-- maintain on the write path. An index that wins against the naive query but
-- loses to a rewrite of the query has not earned a place in the schema.
--
-- The arithmetic also limits how much a BRIN could ever win here. The average
-- partition is 292 pages and the largest is 505, so at the default 128 pages
-- per range a partition holds two to four BRIN ranges and there is very
-- little for range exclusion to throw away.
--
-- The full head to head, with both plans and the buffer counts, is case 5 and
-- case 5b in PERFORMANCE.md. It is not restated here.
--
-- BRIN still earns its place on the unpartitioned raw landing tables, which
-- are physically ordered by arrival and are the shape BRIN is built for, and
-- that is where 01 puts it.
-- ---------------------------------------------------------------------
...
-- DELETED. THE MOST EXPENSIVE THING IN THIS FILE WAS THE ONE NOTHING USED.
-- There was a partial covering index here:
--   CREATE INDEX ix_fct_event_terminal ON mart.fct_deal_stage_event
--       (to_stage_sk, event_date) INCLUDE (deal_value_zar)
--       WHERE is_terminal_event;
-- with three paragraphs above it explaining that it served win and loss
-- reporting, why partial was right, and why INCLUDE beat a fourth key column.
-- Every one of those paragraphs was sound reasoning about an index that was
-- never once used. The index usage check in 06_performance.sql section 9
-- measured zero scans across all 85 index relations, at 14 MB, after a full
-- load and the entire analytics suite.
--
-- WHY NOTHING USED IT: the queries that read terminal events (Q09 and Q02)
-- do not filter to them. Q09 reads every event for every deal because it has
-- to rank re-entries before it can pick the definitive one, and Q02 walks the
-- whole funnel. A partial index only helps a query that carries the partial
-- predicate, and neither of them does. The index was designed for the query I
-- imagined rather than the one I wrote.
--
-- It is deleted rather than kept "in case", because 14 MB of index that no
-- plan touches is 14 MB the write path maintains on every load for nothing.
--
-- TODO: write the win and loss query this index was designed for, then add
-- the index back with a usage count next to the justification instead of a
-- paragraph of reasoning. The query would be "terminal events by month and
-- outcome, with value", which the reporting layer will eventually want and
-- which nothing in 05_analytics.sql currently asks. Adding the index first
-- and the query never is how this got here the first time.
...
--   At 248,940 rows there was never a performance case for partitioning it
--   either. Unpartitioned, PRIMARY KEY (deal_nk) enforces the true grain and
--   a restatement updates in place.
--
--   ON THE SIZE, AND SAY WHICH NUMBER YOU MEAN. Measured on this database
--   after one full ./run.sh: 53 MB heap, 85 MB including indexes, 248,940
--   live rows and zero dead ones. n_tup_upd is 0 on a first build, because
--   the MERGE below finds nothing MATCHED and every row takes the INSERT
--   branch. The UPDATE branch is what a SECOND load exercises, and that is
--   the whole point of the accumulating snapshot: milestones land later than
--   the deal does. So the heap can be expected to grow on re-load, because
--   MVCC writes a new tuple version per update and autovacuum reclaims the
--   old one for reuse inside the file rather than handing the pages back to
--   the filesystem. That growth is the design working, not bloat to chase.
--   The only figures quoted here are the ones this build produces.
--
